...
Optional components (echo, list, poem, itin, textform) come from core's
registry via `book.json` `components`; `css/core.css`, `css/components.css`,
`data/components.json` and `components-reference.md` are generated by the
build. The book's own look is `css/theme.css`.
